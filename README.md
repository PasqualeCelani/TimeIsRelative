...
4. **Time Approximation:** This final step is arguably the most problematic as it converts the geometric angles into time. The general idea is to subdivide the $360^{\circ}$ clock circle into Pizza slices where $360^{\circ} / 60 \text{ minutes} = 6^{\circ} \text{ per minute}$ and $360^{\circ} / 12 \text{ hours} = 30^{\circ} \text{ per hour}$. However, a challenge arises because the arctan2 function calculates $0^{\circ}$ from the 3 o'clock position, meaning a hand pointing to 15 minutes would incorrectly return $0^{\circ}$. To fix this I just apply a translation/normalization by adding $90^{\circ}$ to the angle. Now, the 3 o'clock hand becomes $0^{\circ} + 90^{\circ} = 90^{\circ}$, and $90 / 6$ correctly yields $15$ minutes. This same logic applies to the hour hand. Finally, two critical edge cases are left to handle the mathematical hour = 0 is translated to hour = 12, and if rounding causes minutes = 60, I just reset the minutes to $0$ and correctly roll the hour over ensuring to be in $[1, 12]$.

## :warning: Limitations
* The methodology operates under the assumption that the image contains a unique clock object;
* The methodology does not incorporate rotation compensation, so is not accurate on rotated clocks;
* The methodology relies on the assumption that the clock hands are rigid straight lines (or at least in part);
* The methodology is restricted to clocks possessing only two hands (hour and minute), if the seconds hand line is present the method will fail to correctly detect the time.
